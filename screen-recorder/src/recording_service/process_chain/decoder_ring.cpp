...
DecoderChainRing::DecoderChainRing(AVStream *inputStream) {
    ...
    decoderContext = std::unique_ptr<AVCodecContext, FFMpegObjectsDeleter>(avcodec_alloc_context3(streamCodec));
    if (!decoderContext) {
    ...
    int ret;
    ret = avcodec_parameters_to_context(decoderContext.get(), inputStream->codecpar);
    if (ret < 0) {
...
void DecoderChainRing::execute(ProcessContext *processContext) {
     auto decodedFrame = std::unique_ptr<AVFrame, FFMpegObjectsDeleter>(av_frame_alloc());
    if (decodedFrame == nullptr) {
        throw std::runtime_error(Error::build_error_message(__FUNCTION__, {}, "error allocating frame"));
    }

    int response = avcodec_send_packet(decoderContext.get(), processContext->sourcePacket.get());
    if (response < 0) {
        throw std::runtime_error(
                Error::build_error_message(__FUNCTION__, {},
    ...
    while (response >= 0) {
        ...
        if (response >= 0) {
            if (std::holds_alternative<std::shared_ptr<FilterChainRing>>(next)) {
                std::get<std::shared_ptr<FilterChainRing>>(next)->execute(processContext, decodedFrame.get());
            } else {
                std::get<std::shared_ptr<EncoderChainRing>>(next)->execute(processContext, decodedFrame.get());
            }
        }
    }
}
